...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
	...
	<xsl:template match="TestScript">
		<html>
			...
			<body>
				...
				<TABLE width="95%">					
					<TR>
						<TD COLSPAN="1" style="width: 200px;"><b>DISPLAY FORMAT:</b></TD><TD COLSPAN="5" style="text-align: left;">Normal</TD>
					</TR>					
					<TR>
	...
	<!-- These are logs we handle manually, and don't want them to be picked up recursively -->
	...
	<xsl:template match="Trace/Location"/> <!-- Already handled in "Trace" template -->	
	<xsl:template match="Trace/AdditionalInfo"/> <!-- Already handled in "Trace" template -->	
	<xsl:template match="Trace/Debug"/>
	<xsl:template match="TestCase/Debug"/>
	...
	<xsl:template match="RetryGroup/GroupName"/> <!-- Already handled in "CheckGroup" template -->
   <xsl:template match="RetryGroup/Result"/> <!-- Already handled in "CheckGroup" template -->
	...
	<xsl:template match="TestScript/Messaging/Location|TestScript/Trace/Location|TestScript/Support/Location"/>
	...
	<xsl:template match="TestCase/Messaging|TestCase/Support|ScriptInit/Messaging|ScriptInit/Support" />
<!--  		<TR bgColor="#ffffff">
			<TD colspan="2">
	...
	<xsl:template match="Trace/Messaging|Trace/Support"/>
<!--		<TR bgColor="#ffffff">
			<TD colspan="2">
				<TABLE width="100%" style="border-left: 1px solid #999999;">
					<TR>
						...
						<TD style="FONT-SIZE: 13px; FONT-WEIGHT: bold; width: 95%;">
							...
							<xsl:choose>
								<xsl:when test="not(string($objname))">
									<xsl:value-of select="Location/Time"/> - <xsl:value-of select="MethodName"/>(<xsl:value-of select="MethodArguments"/>)
								</xsl:when>
								<xsl:otherwise>
									<xsl:value-of select="Location/Time"/> - <xsl:value-of select="ObjectName"/>.<xsl:value-of select="MethodName"/>(<xsl:value-of select="MethodArguments"/>)
								</xsl:otherwise>
							</xsl:choose>
						</TD>						
					</TR>
		...
		<xsl:choose>
			<xsl:when test="contains($cpResult,'PASSED')">
					<TR style="background-color: #DEDEDE;">
						...
						<TD style="FONT-SIZE: 13px;"><xsl:value-of select="Expected"/></TD>
						<TD style="FONT-SIZE: 13px;"><xsl:value-of select="Actual"/></TD>
						<TD style="FONT-SIZE: 13px; FONT-WEIGHT: bold;">PASS</TD>
					</TR>
				</xsl:when>
				<xsl:otherwise>
					<TR style="background-color: #FFA9A9;">
						...
						<TD style="FONT-SIZE: 13px;"><xsl:value-of select="Expected"/></TD>
						<TD style="FONT-SIZE: 13px;"><xsl:value-of select="Actual"/></TD>
						<TD style="FONT-SIZE: 13px; FONT-WEIGHT: bold;">FAIL</TD>
					</TR>
				</xsl:otherwise>
			</xsl:choose>
	</xsl:template>
...
<xsl:template match="Messaging/Location|Support/Location" />	
<!-- 				<TR>
					<TD style="FONT-SIZE: 13px; width: 50px;">
						<b>Time:</b>
					</TD>
