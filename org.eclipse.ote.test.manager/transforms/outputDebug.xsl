...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
	...
	<xsl:template match="TestScript">
		<html>
			...
			<body>
				...
				<TABLE width="95%">					
					<TR>
						<TD COLSPAN="1" style="width: 200px;"><b>DISPLAY FORMAT:</b></TD><TD COLSPAN="5" style="text-align: left;">Debug</TD>
					</TR>					
					<TR>
					...
					<TR>
						...
						<xsl:if test="string($modifiedFlag) and not($modifiedFlag = '-')"> - <SPAN style="color: red;"><xsl:value-of select="$modifiedFlag" /></SPAN></xsl:if>
						</TD>
					</TR>	
					<TR>
	...
	<xsl:template match="TestScript/Debug/Location|TestScript/Messaging/Location|TestScript/Trace/Location|TestScript/Support/Location"/>
   ...
   <xsl:template match="RetryGroup/Result"/> <!-- Already handled in "CheckGroup" template -->
	<xsl:template match="UutLoggingInfo"/> <!-- Already handled in "CheckGroup" template -->
   ...
   <xsl:template match="ExecutionStatus|TestCase/ExecutionStatus">
	...
	<xsl:template match="Debug/Location|Messaging/Location|Support/Location">
		<TABLE width="100%">					
				<TR>
					<TD style="FONT-SIZE: 13px; width: 50px;">
						<b>Time:</b>
