...
public class CharElement extends DiscreteElement<Character> {
   ...
   public CharElement(Message message, String elementName, MessageData messageData, int byteOffset, int msb,
      int lsb) {
   ...
   public CharElement(Message message, String elementName, MessageData messageData, int byteOffset, int msb, int lsb,
      int originalLsb, int originalMsb) {
   ...
   public boolean checkNot(ITestAccessor accessor, CheckGroup checkGroup, String value,
      ...
      if (milliseconds > 0) {
         ...
         org.eclipse.osee.ote.core.environment.interfaces.ICancelTimer cancelTimer = accessor.setTimerFor(listener,
            milliseconds);
         ...
         while (result = !(currentValue = getString(accessor, value.length())).equals(value)) {
            ...
            // expires
            /*
             * NOTE: had to add isTimedOut() because we were getting data at the same time we're timing out, so
             * the notifyAll() isn't guaranteed to work since we would not be in a waiting state at that time -
             * so we're forced to save the fact that we timed out.
             */
   ...
   public boolean check(ITestAccessor accessor, CheckGroup checkGroup, String value,
      ...
      time = accessor.getEnvTime() - time;
      CheckPoint passFail = new CheckPoint(this.getFullName(), value, currentValue,
         compareString(currentValue, value), time);
   ...
   public boolean checkNot(ITestAccessor accessor, CheckGroup checkGroup, String value) {
      ...
      CheckPoint passFail = new CheckPoint(this.getFullName(), "Not " + value, actualValue,
         value.compareTo(actualValue) != 0, 0);
   ...
   public boolean check(ITestAccessor accessor, CheckGroup checkGroup, String value) {
      ...
      CheckPoint passFail = new CheckPoint(this.getFullName(), value, actualValue, value.compareTo(actualValue) == 0,
         0);
   ...
   /**
    * <p>
    * Sets the element's value. Can be [ab]used to set subsequent, contiguous bytes but, while
    * widespread, this use is discouraged and may be removed in a future release. The behavior of this
    * method differs depending on the length of string passed in and whether the underlying element is
    * eight bits.
    * </p>
    * <p>
    * For an empty string it sets the element to the null character, '\0', using
    * {@link #setValue(Character)}, which properly accounts for cases where the element is not eight
    * bits.
    * </p>
    * <p>
    ...
    * </p>
    * <p>
    * If the string length is greater than one it will assume this and all subsequent elements are
    * eight bits and set them. <b>Note</b> that it does nothing to ensure the subsequent bytes are
    * character elements or eight bits. It will throw IllegalArgumentException if setting subsequent
    * elements would overflow. If the element is not eight bits in size it will set as above and log a
    * warning and stack trace but not throw an exception.
    * </p>
    *
    * @param accessor
   ...
   /**
    * This method uses {@link #parseAndSet(ITestEnvironmentAccessor, String)} and has all the associate
   ...
   /**
    * This method uses {@link #parseAndSet(ITestEnvironmentAccessor, String)} and has all the associate
    * risks and limitations. After setting it sends the message.
    *
    * @param accessor
    * @param value
    */
    ...
    */
   protected String waitForValue(ITestEnvironmentAccessor accessor, String value,
      int milliseconds) throws InterruptedException {
      ...
      String currentValue;
      if (milliseconds > 0 && accessor != null) {
         MessageSystemListener listener = getMessage().getListener();
         org.eclipse.osee.ote.core.environment.interfaces.ICancelTimer cancelTimer = accessor.setTimerFor(listener,
            milliseconds);
    ...
    * @return last value found. Either value expected or value found at timeout.
    */
   protected String waitForNotValue(ITestEnvironmentAccessor accessor, String value,
      int milliseconds) throws InterruptedException {
