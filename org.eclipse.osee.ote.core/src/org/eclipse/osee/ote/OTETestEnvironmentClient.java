...
public class OTETestEnvironmentClient {
   ...
   public static OTETestEnvironmentClient getInstance(String endpoint) {
      return new OTETestEnvironmentClient(ServiceUtility.getService(OteUdpEndpoint.class),
   ...
   public ICommandHandle runScripts(UUID sessionId, PropertyStore globalProperties, List<IPropertyStore> scriptProperties, boolean useOTEBuilderJars, File rootFolderForOTEBuilderJars, SubProgressMonitor monitor, String serverId, ConfigurationStatusCallback configurationStatusCallback) throws URISyntaxException, InterruptedException, ExecutionException, IOException {
      ...
      if (useOTEBuilderJars) {
         configuration =
            configureEnvironment(serverId, rootFolderForOTEBuilderJars, monitor, configurationStatusCallback);
      }
      RunTests runTests =
         new RunTests(UUID.randomUUID().toString(), sessionId, configuration, globalProperties, scriptProperties);

      OteEventMessageUtil.postEvent(new RunTestsSerialized(runTests));
      return new EventRunTestsHandle(runTests.getGUID(),
         new OteSendEventMessage(ServiceUtility.getService(EventAdmin.class)));
   }
   ...
   public Configuration configureEnvironment(String serverId, final List<File> bundlesToSend, final boolean installBundles, final SubProgressMonitor monitor, final ConfigurationStatusCallback configurationStatusCallback) {
      ...
      try {
         ...
         classServer = new HeadlessClassServer(PortUtil.getInstance().getValidPort(),
            OteProperties.getDefaultInetAddress(), bundlesToSend);
         for (BundleInfo bundleInfo : classServer.getBundles()) {
            ConfigurationItem item = new ConfigurationItem(bundleInfo.getServerBundleLocation().toString(),
               bundleInfo.getVersion(), bundleInfo.getSymbolicName(), bundleInfo.getMd5Digest(), installBundles);
            localConfiguration.addItem(item);
         }

         if (service != null) {
            InetSocketAddress address = service.getLocalEndpoint();
            ConfigurationAndResponse configAndResponse = new ConfigurationAndResponse(
               address.getAddress().getHostAddress(), address.getPort(), localConfiguration, installBundles, serverId);
            ...
            final OteEventMessageFuture<SerializedConfigurationAndResponse, SerializedOTEJobStatus> asynchSendAndResponse =
               sendit.asynchSendAndMultipleResponse(SerializedOTEJobStatus.class, ser, completion, 1000 * 60 * 10);
   ...
   private Configuration configureEnvironment(String serverId, final File oteBuilderRootFolder, final SubProgressMonitor monitor, final ConfigurationStatusCallback configurationStatusCallback) {
      ...
      try {
         ...
         for (File jar : jars) {
            try {
               ConfigurationItem item = new ConfigurationItem(classServer.getHostName() + jar.getName(), "1.0",
                  jar.getName(), Lib.fileToString(new File(jar.getAbsolutePath() + ".md5")), false);
               localConfiguration.addItem(item);
         ...
         if (service != null) {
            InetSocketAddress address = service.getLocalEndpoint();
            ConfigurationAndResponse configAndResponse = new ConfigurationAndResponse(
               address.getAddress().getHostAddress(), address.getPort(), localConfiguration, false, serverId);
            SerializedConfigurationAndResponse ser = new SerializedConfigurationAndResponse();
            ser.setObject(configAndResponse);
            ser.getHeader().RESPONSE_TOPIC.setValue(SerializedOTEJobStatus.EVENT);
   ...
   public ConnectionRequestResult requestEnvironment(IRemoteUserSession session, UUID id, TestEnvironmentConfig config) throws RemoteException {
      ...
      try {
         SerializedRequestRemoteTestEnvironment req =
            new SerializedRequestRemoteTestEnvironment(new RequestRemoteTestEnvironment(session, id, config));
         SerializedConnectionRequestResult result = new SerializedConnectionRequestResult();
         OteEndpointSendEventMessage sendit = new OteEndpointSendEventMessage(service, destinationAddress);
         result =
            sendit.synchSendAndResponse(result, req, OtePropertiesCore.serverConnectionTimeout.getLongValue(1000 * 30));
         if (result != null) {
   ...
   public void disconnect(UUID sessionId) throws RemoteException {
      try {
         SerializedDisconnectRemoteTestEnvironment req =
            new SerializedDisconnectRemoteTestEnvironment(new DisconnectRemoteTestEnvironment(sessionId));
         OteEndpointSendEventMessage sendit = new OteEndpointSendEventMessage(service, destinationAddress);
